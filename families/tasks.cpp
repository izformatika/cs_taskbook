...
int age_between_f(std::ostream &fs)
{
    ...
    fs << "Укажите ID человека, на момент рождения которого его матери было от "<<age_f<<(age_f%10==1?" полного года":" полных лет")<<" включительно до " << age_t<<(age_t%10==1?" полного года":" полных лет") << " включительно. Месяцем и днём можно пренебречь - считайте, что все люди родились в один день. Если таких людей несколько, укажите наименьший ID." << std::endl;
    return res;
}
